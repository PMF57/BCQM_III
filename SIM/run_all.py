...
def fig1_trajectories():
    ...
    _style_axes(ax, "Fig. 1  Trajectory ensembles at b=0", "t", "x(t)")
    ax.legend()
...
def fig2_mean_displacement():
    ...
    _style_axes(ax, "Fig. 2  Mean displacement at b=0", "t", "mean Δx(t)")
...
def fig3_meff_vs_Wcoh():
    ...
    _style_axes(ax, "Fig. 3  m_eff vs Wcoh", "Wcoh", "m_eff (est.)")
    ax.legend()
...
def fig4_autocorr_collapse():
    ...
    _style_axes(ax, "Fig. 4  Velocity autocorrelation collapse", "lag / Wcoh", "C_v(lag)")
    ax.legend()
